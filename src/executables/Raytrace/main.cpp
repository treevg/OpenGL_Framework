...
int main(int argc, char *argv[]) {
    ...
    new ShaderProgram({"/Filters/fullscreen.vert","/Filters/toneMapperLinear.frag"})))
        ->texture("tex", raytracePass->get("fragColor"))
    ...
    setKeyCallback(window, [&] (int key, int scancode, int action, int mods) {
        if (action == GLFW_PRESS || action == GLFW_REPEAT) {
            switch (key) {
            case GLFW_KEY_1:
                tonemapping->texture("tex", raytracePass->get("fragColor"));
                break;
            case GLFW_KEY_2:
                tonemapping->texture("tex", raytracePass->get("fragPosition"));
                break;
            case GLFW_KEY_3:
                tonemapping->texture("tex", raytracePass->get("indirectColor"));
                break;
            case GLFW_KEY_4:
                tonemapping->texture("tex", raytracePass->get("fragPosition2"));
                break;
            case GLFW_KEY_5:
