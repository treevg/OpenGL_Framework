...
bool warpping;
double lasttime;
vec3 lightPosition = vec3(129,300,200);
...
CollisionObject* test1;
CollisionCollection* collController;
...
static void  moveWithKeybord(float deltaTime, bool collided)
{

	camera->moveWithKey(window, speed*deltaTime, collided);
	quit();
...
void GameDesktop::setMatrix()
{
	...
	modelWindMill = scale(modelWindMill, vec3(0.03, 0.03, 0.03));
	...
	modelWell =  scale(modelWell, vec3(0.03, 0.03, 0.03));
	...
	modelPig = translate(modelPig, vec3(120.88, -6.0, 150.05));
...
void GameDesktop::initCollisionController()
{

	
//test = new CollisionObject();
 //test  =  new CollisionObject();
cout<< "----------- preparing collision controller----------------"  << endl;
cout << meshes.size() << endl;
 collController = new CollisionCollection();

collController->add(vikingMeshes, vikingModel, "viking");
//collController->add(castleMeshes, modelCastle, "castle");
collController->add(vikingMeshes, vikingModel2, "viking2");
collController->add(windMillMeshes, modelWindMill, "windmill");
collController->add(house1Meshes, modelHouse1, "house1");
collController->add(house1Meshes, modelHouse6, "house1");
collController->add(house1Meshes, modelHouse9, "house1");
collController->add(house2Meshes, modelHouse2, "house2");
collController->add(house2Meshes, modelHouse7, "house2");
collController->add(house3Meshes, modelHouse3, "house3");
collController->add(house3Meshes, modelHouse4, "house3");
collController->add(house3Meshes, modelHouse5, "house3");
collController->add(house3Meshes, modelHouse8, "house3");
collController->add(lowPollyHouseMeshes, lowHouse1, "lowpolyhouse");
collController->add(lowPollyHouseMeshes, lowHouse2, "lowpolyhouse");
collController->add(lowPollyHouseMeshes, lowHouse3, "lowpolyhouse");
collController->add(lowPollyHouseMeshes, lowHouse4, "lowpolyhouse");
collController->add(lowPollyHouseMeshes, lowHouse5, "lowpolyhouse");
collController->add(lowPollyHouseMeshes, lowHouse6, "lowpolyhouse");
collController->add(lowPollyHouseMeshes, lowHouse7, "lowpolyhouse");
collController->add(lowPollyHouseMeshes, lowHouse8, "lowpolyhouse");
collController->add(lowPollyHouseMeshes, lowHouse9, "lowpolyhouse");
collController->add(wellMeshes, modelWell, "well");
collController->add(cartMeshes, modelCart, "cart");
collController->add(pigMeshes, modelPig, "pig");
collController->add(smaugMeshes, modelSmaug, "smaug" );
collController->add(meshes, modelTree,  "tree" );
collController->add(meshes, modelTree1, "tree" );
collController->add(meshes, modelTree2, "tree" );
collController->add(meshes, modelTree3, "tree" );
collController->add(meshes, modelTree4, "tree" );
collController->add(meshes, modelTree5, "tree" );
collController->add(meshes, modelTree6, "tree" );
collController->add(meshes, modelTree7, "tree" );
collController->add(meshes, modelTree8, "tree" );
...
void GameDesktop::init()
{
	/* Camera  */
	window = generateWindow();
	camera =  new Camera();
	player = new Player(camera->getPosition(), 1.5f);
...
// other shader
	...
	windMill = new Model(RESOURCES_PATH "/uwindmill.obj", modelWindMill, modSP);
	...
	house2 = new Model(RESOURCES_PATH "/house_medieval_1.obj", modelHouse2, modSP);
...
GameDesktop::GameDesktop(bool warp)
{
...
void  GameDesktop::renderSzene()
{
	...
	render(window,  [&] (float deltaTime)
	{
        ...
        bool collided = collController->checkCollision(player);
        moveWithKeybord(deltaTime, collided);
...
int main(int argc, char *argv[])
{
	...
	GameDesktop * g = new GameDesktop(argc > 1 );
